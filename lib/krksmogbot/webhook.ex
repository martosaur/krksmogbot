defmodule Krksmogbot.Webhook do
  use Plug.Builder
  ...
  def setup_webhook do
    ...
    case Application.get_env(:krksmogbot, :use_webhook) do
      "true" ->
        host = Application.fetch_env!(:krksmogbot, :webhook_url)
        path = Application.fetch_env!(:krksmogbot, :webhook_token)
        max_connections = Application.get_env(:krksmogbot, :webhook_max_connections, 40)
        url = host <> path
  ...
  defp check_webhook_enabled(conn, _opts) do
    if Application.get_env(:krksmogbot, :use_webhook) != "true" do
      conn
  ...
  defp check_token(conn, _opts) do
    if conn.params["token"] == Application.fetch_env!(:krksmogbot, :webhook_token) do
      conn
    else
      conn
      |> send_resp(404, "Page not found")
  ...
  defp invoke_processor(%{assigns: %{update: %Nadia.Model.Update{} = update}} = conn, _opts) do
    Task.start(Krksmogbot.Processor, :process_message, [update])
    conn
  end
